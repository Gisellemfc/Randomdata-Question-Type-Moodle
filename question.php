...
/**
 * Random Data question definition class.
 *
...
/**
 * Represents a randomdata question.
 *
 * @copyright  2022 Nicole Brito nicole.brito@correo.unimet.edu.ve, Giselle Ferreira giselle.ferreira@correo.unimet.edu.ve
 * @copyright  Based on work 2011 The Open University
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_randomdata_question extends qtype_numerical_question
        implements qtype_randomdata_question_with_expressions {

    /** @var qtype_randomdata_dataset_loader helper for loading the dataset. */
    public $datasetloader;

    /** @var qtype_randomdata_variable_substituter stores the dataset we are using. */
    public $vs;
    ...
    /**
     * @var array of formula validations.
     */
    public $formula_validations;


    public function start_attempt(question_attempt_step $step, $variant) {
        qtype_randomdata_question_helper::start_attempt($this, $step, $variant);
        parent::start_attempt($step, $variant);
    }


    public function apply_attempt_state(question_attempt_step $step) {
        qtype_randomdata_question_helper::apply_attempt_state($this, $step);
    ...
    public function calculate_all_expressions() {
        ...
        foreach ($this->answers as $ans) {
            ...
            if ($ans->answer && $ans->answer !== '*') {
                $ans->answer = $this->vs->calculate($ans->answer, $ans->correctanswerlength, $ans->correctanswerformat);
            }

            $ans->feedback = $this->vs->replace_expressions_in_text($ans->feedback, $ans->correctanswerlength, $ans->correctanswerformat);
        }
    }
    ...
    public function get_correct_response() {
        ...
        $response = array('answer' => $this->vs->format_float($answer->answer, $answer->correctanswerlength, $answer->correctanswerformat));
 ...
 * is to work with {@link qtype_randomdata_question_helper}.
 *
 * As well as this method, the class that implements this interface must have fields
 * public $datasetloader; // of type qtype_randomdata_dataset_loader
 * public $vs; // of type qtype_randomdata_variable_substituter
 *
 * @copyright  2022 Nicole Brito nicole.brito@correo.unimet.edu.ve, Giselle Ferreira giselle.ferreira@correo.unimet.edu.ve
 * @copyright  Based on work 2011 The Open University
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
interface qtype_randomdata_question_with_expressions {
    /**
 ...
 * {@link qtype_randomdata_question_with_expressions} and the class
 * {@link qtype_randomdata_dataset_loader} to set up the value of each variable
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
abstract class qtype_randomdata_question_helper {

    public static function start_attempt(qtype_randomdata_question_with_expressions $question, question_attempt_step $step, $variant) {

        $question->vs = new qtype_randomdata_variable_substituter( $question->datasetloader->get_values($variant), get_string('decsep', 'langconfig'));
    ...
    public static function apply_attempt_state(qtype_randomdata_question_with_expressions $question, question_attempt_step $step) {
        $values = array();
        ...
        $question->vs = new qtype_randomdata_variable_substituter($values, get_string('decsep', 'langconfig'));
        $question->calculate_all_expressions();
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_randomdata_dataset_loader {
...
/**
 * This class holds the current values of all the variables used by a randomdata
 * question.
 *
 * It can compute formulate using those values, and can substitute equations
 * embedded in text.
 *
 * @copyright  2022 Nicole Brito nicole.brito@correo.unimet.edu.ve, Giselle Ferreira giselle.ferreira@correo.unimet.edu.ve
 * @copyright  Based on work 2011 The Open University
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_randomdata_variable_substituter {
    ...
    public function __construct(array $values, $decimalpoint) {
        ...
        foreach ($values as $name => $value) {
            if (!is_numeric($value)) {
                ...
                $a->name = '{' . $name . '}';
                $a->value = $value;
                throw new moodle_exception('notvalidnumber', 'qtype_randomdata', '', $a);
            }
    ...
    public function calculate($expression) {
        ...
        if ($error = qtype_randomdata_find_formula_errors($expression)) {
            throw new moodle_exception('illegalformulasyntax', 'qtype_randomdata', '', $error);
        }
        $expression = $this->substitute_values_for_eval($expression);
        if ($datasets = question_bank::get_qtype('randomdata')->find_dataset_names($expression)) {
            // Some placeholders were not substituted.
            throw new moodle_exception('illegalformulasyntax', 'qtype_randomdata', '',
                '{' . reset($datasets) . '}');
        }
    ...
    protected function calculate_raw($expression) {
        ...
        throw new moodle_exception('illegalformulasyntax', 'qtype_randomdata', '', $expression);
    }
    ...
    public function replace_expressions_in_text($text, $length = null, $format = null) {
        ...
        $text = preg_replace_callback(qtype_randomdata::FORMULAS_IN_TEXT_REGEX,
