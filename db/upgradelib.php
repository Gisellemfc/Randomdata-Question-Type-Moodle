...
/**
 * Upgrade library code for the randomdata question type.
 *
...
/**
 * Class for converting attempt data for randomdata questions when upgrading
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class qtype_randomdata_qe2_attempt_updater extends question_qtype_attempt_updater {
    ...
    protected function parse_response($state) {
        if (strpos($state->answer, '-') < 7) {
            ...
            throw new coding_exception("Brokes state {$state->id} for randomdata
        ...
        if (is_null($this->selecteditem)) {
            ...
            $this->logger->log_assumption("Different states for randomdata question
     ...
     * {@link qtype_randomdata_variable_substituter::format_float()}. Except that
